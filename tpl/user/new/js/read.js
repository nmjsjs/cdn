...
function login_form() {
    ...
    $("#login").click(function(e) {
	    if ("用户名" == $("#user_name").val()) $.showfloatdiv({
		     txt: "请输入正确的用户名"
	    }), $("#user_name").focus(), $.hidediv({});
	    else {
		if ("" != $("#user_password").val()) return $("#loginform").qiresub({
			curobj: $("#login"),
			txt: "数据提交中,请稍后...",
			onsucc: function(a) {
				if ($.hidediv(a), parseInt(a["code"]) > 0) {
...
function pagegoo(url) {
	$.ajax({
		...
		success: function(data) {
			if (data != '') {
				if ($('#commbox ul').length > 3) $("html,body").animate({
				...
				$(".replyt").click(function(e) {
					...
					if (!checkcookie()) {
						...
						if ($("#reps" + curid).html() != '') {
							...
							$("#reps" + curid + " #comm_tid").val(curtid); //回贴ID
							$("#reps" + curid + " #comm_tuid").val(curtuid); //回贴用户ID
							$("#reps" + curid + " #comm_sid").val(cursid);
							$("#reps" + curid + " #comm_vid").val(curvid);
